...
class WarrantyMigrateController extends Controller
{
    public function index()
    {
        ...
        foreach ($this->masterlist as $key => $value) {
            ...
            // de($warranty->where('vehicle_id', 451)->first());
            if ($_vehicle) {
                // echo $value . "\n";
                // echo $_vehicle['id'] . "\n";
                $_warranty = $warranty->where('vehicle_id', $_vehicle['id'])->first();
        ...
        // //completed warranty
        // $completed_warranty = $warranty->where('status', 'completed');
        // $success = 0;
        // $error = 0;
        // foreach ($completed_warranty as $key => $value) {
        //     echo "Migrating completed warranty $key..";
        //     try {
        //         DB::transaction(function () use ($value, $relation_data) {
        //             $this->insertWarranty((object)$value, $relation_data);
        //         });
        //         DB::commit();
        //         $success++;
        //         echo "done $key \n";
    ...
    private function insertWarranty($data, $relation_data, $masterlist = null)
    {
        ...
        $new_vehicle = $this->createVehicle($vehicle, $proposer);
    ...
    private function createWarranty($warranty_data, $data, $vehicle, $masterlist = null)
    {
        ...
        if (!$masterlist) {
            ...
            $warranty->ci_no = $masterlist->where('vehicle_no', $vehicle->registration_no)->first()['ci_no'];
            $warranty->save();
        }
    ...
    private function createVehicle($data, $proposer)
    {
        $vehicle = Vehicle::where('registration_no', $data->registration_no)->first();
    ...
    {
        $dealer = Company::where('code', $data->code)->first();
        if (!$dealer) {
    ...
    {
        $company = Company::where('name', 'India International Insurance Pte Ltd')->first();
    ...
    private function normalizeCIFolder($folder)
    {
        $data = [];

        foreach ($folder as $key => $value) {
            $explode_name = explode(' ', $value);
            // echo explode('.pdf', $explode_name[2])[0] . "\n";
            $data[$key] = explode('.pdf', $explode_name[2])[0];
        }
